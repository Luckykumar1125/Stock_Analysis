...
// ----------------------------------------------------------------------
// --- CONFIGURATION & TYPES (Kept the same) ---
// ----------------------------------------------------------------------
...
// ----------------------------------------------------------------------
// --- UTILITY COMPONENTS (Kept the same) ---
// ----------------------------------------------------------------------
    ...
    () => [
      {
        ...
        Cell: ({ row }) => (
            <div className="flex flex-col space-y-0">
                <p className="font-semibold text-foreground">{row.original.company}</p>
                <p className="text-xs text-muted-foreground">{row.original.ticker}</p>
            </div>
        )
      },
      {
      ...
      },
      {
        Header: 'Mkt Cap',
        accessor: 'market_cap',
        Cell: ({ value }) => <span className='text-sm text-muted-foreground'>{formatNumber(value)}</span>,
      },
      {
        Header: '52W High',
        accessor: '52_week_high',
  ...
  if (data.length === 0) {
    return (
      <div className="p-8 text-center border rounded-lg border-dashed">
        <p className="text-muted-foreground">No stock data available for this sector.</p>
      </div>
    );
  }
              ...
              {headerGroup.headers.map((column) => (
                ...
                >
                  <div className="flex items-center gap-1.5">
                    ...
                    {column.canSort && (
                      column.isSorted ? (
                        column.isSortedDesc ? <ArrowDown className="h-3 w-3" /> : <ArrowUp className="h-3 w-3" />
                      ) : (
...
const StockScreener: React.FC<StockScreenerProps> = (props) => {
  const [selectedSector, setSelectedSector] = useState<string | undefined>(undefined); 
  const [results, setResults] = useState<ScreenerResults | null>(null);
  const [isLoading, setIsLoading] = useState(false);
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      if (data.status !== 'success') {
        toast.error('Search Failed', { description: data.message });
      } else {
        toast.success(`Found ${data.total_stocks} stocks`);
      }
  ...
  return (
    <div className="flex flex-col gap-6 w-full h-full text-foreground">
      ...
      <div className="p-6 bg-card border rounded-xl shadow-sm">
        <div className="flex flex-col sm:flex-row gap-4 items-end">
          <div className="flex-1 w-full space-y-2">
            <Label htmlFor="sector-select" className="text-base font-medium">
                Select Market Sector
            </Label>
            
            <Select onValueChange={setSelectedSector} value={selectedSector}>
              <SelectTrigger id="sector-select" className="w-full h-11">
          ...
          <Button
            onClick={handleSearch}
            disabled={isLoading || !selectedSector}
            className="h-11 px-8 min-w-[140px] gradient-primary"
          >
            {isLoading ? (
              <>
                <Loader2 className="mr-2 h-4 w-4 animate-spin" />
      ...
      <div className="flex-1 min-h-[300px]">
          {isLoading && (
            <div className="flex flex-col items-center justify-center h-64 space-y-4 animate-in fade-in slide-in-from-bottom-4 duration-500">
          ...
          {!isLoading && !results && (
             <div className="flex flex-col items-center justify-center h-64 text-muted-foreground border-2 border-dashed rounded-xl">
                ...
                <p>Select a sector above to start screening</p>
             </div>
          )}
          ...
          {results && !isLoading && (
            <div className="animate-in fade-in duration-500">
                <div className="flex items-center justify-between mb-4">
                    ...
                    <Badge variant="secondary">{results.total_stocks} found</Badge>
                </div>
                <StockDataDisplay data={results.stocks.filter(s => !s.error)} />
            </div>
          )}
